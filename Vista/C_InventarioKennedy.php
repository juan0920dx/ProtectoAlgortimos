        ...
        <div class="inventario">
            <ul>
                ...
                while ($row = mysqli_fetch_assoc($resultado)) {
                    echo "<li>" . $row['productoK'] . " - Cantidad: " . $row['cantidadK'] . "</li>";
                }
        ...
        <form id="ingreso-producto" action="#" method="POST">
            <div class="formulario">
                ...
                <select id="producto" name="producto">
                    ...
                    $resultado = mysqli_query($conexion, $sql);
                    while ($row = mysqli_fetch_assoc($resultado)) {
                        echo "<option value='" . $row['idProductoK'] . "' data-imagen='" . $row['imagenK'] . "'>" . $row['productoK'] . "</option>";
                    }
                    mysqli_close($conexion);
                    ?>
                </select>
                ...
                <div class="botones">
                    ...
                    <button type="button" class="boton-volver" onclick="window.location.href='cajero_kennedy.php'">Volver</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    document.getElementById("actualizar-cantidad").addEventListener("click", function() {
        ...
        fetch("../Controlador/C_ActualizarK.php?productoId=" + encodeURIComponent(productoId) + "&cantidad=" + encodeURIComponent(cantidad))
            .then(response => {
                if (response.ok) {
