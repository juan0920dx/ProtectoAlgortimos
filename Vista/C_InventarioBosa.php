        ...
        <div class="inventario">
            <ul>
                ...
                while ($row = mysqli_fetch_assoc($resultado)) {
                    echo "<li>" . $row['productoB'] . " - Cantidad: " . $row['cantidadB'] . "</li>";
                }
        ...
        <form id="ingreso-producto" action="#" method="POST">
            <div class="formulario">
                ...
                <select id="producto" name="producto">
                    ...
                    $resultado = mysqli_query($conexion, $sql);
                    while ($row = mysqli_fetch_assoc($resultado)) {
                        echo "<option value='" . $row['idProductoB'] . "' data-imagen='" . $row['imagenB'] . "'>" . $row['productoB'] . "</option>";
                    }
                    mysqli_close($conexion);
                    ?>
                </select>
                ...
                <div class="botones">
                    ...
                    <button type="button" class="boton-volver" onclick="window.location.href='cajero_bosa.php'">Volver</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    document.getElementById("actualizar-cantidad").addEventListener("click", function() {
        ...
        fetch("../Controlador/C_ActualizarB.php?productoId=" + encodeURIComponent(productoId) + "&cantidad=" + encodeURIComponent(cantidad))
            .then(response => {
                if (response.ok) {
